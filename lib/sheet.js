...
function Sheet (sheets, opts) {
  ...
  this.metadata = {
    ...
    schema: opts.schema || require('./schemas/sheet-schema.json'),
...
Sheet.prototype.addRow = function (row, cb) {
  ...
  properties = extend(this.metadata.schema.properties.rows.items.properties, properties)
...
Sheet.prototype.getRow = function (key, cb) {
  ...
  this.dat.get(key, function (err, row) {
    if (err) return cb(err)
    row.properties = self.metadata.schema.properties.rows.items.properties
    cb(null, row)
  })
}
