...
Permissions.prototype.authorizeSession = function (req, res, cb) {
  this.server.getAccountBySession(req, function (err, user, session) {
    if (err) {
      console.log("Permissions.authorizeSession: error:", err);
...
Permissions.prototype.sheetAccessible = function (sheet, account) {
  ...
  if (!sheet.private) return true;
  var editor = Object.keys(sheet.editors).indexOf(account.username) >= 0;
  var owner = Object.keys(sheet.owners).indexOf(account.username) >= 0;
