...
// returns the email's associated account by calling cb(err, account)
AccountIndexes.prototype.getKeyFromEmail =function (email, cb) {
  var opts = [ email ];
  this.indexes.email.findOne(opts, cb);
}

// returns the username's associated account by calling cb(err, account)
AccountIndexes.prototype.getKeyFromUsername =function (username, cb) {
  var opts = [ username ];
  this.indexes.username.findOne(opts, cb);
}
