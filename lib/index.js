...
function Server (db, opts) {
  ...
  console.log("Flatsheet.init: initializing flatsheet...")
...
Server.prototype.getAccountBySession = function (req, cb) {
  ...
  this.auth.getSession(req, function (sessionError, session) {
    ...
    self.accounts.get(session.data.key, function (accountError, account) {
      if (accountError) {
        console.log("server.getAccountBySession: accountError:", accountError)
        return cb(accountError)
      }
      if (!account) {
        console.log("server.getAccountBySession: no account")
        return cb(null, null, session)
      }
...
Server.prototype.listen = function (port, cb) {
  ...
  var tokens = require('../routes/tokens')(this)
  ...
  function handler (req, res) {
    ...
    if (sheetsAPIv30.match(req, res)) return
